...
void logconf_setup_sb(struct logconf *conf, const char *id, char *buf, size_t buf_size)
{
...
void logconf_setup(struct logconf *conf, const char id[], FILE *fp) {
  size_t buf_size = 0;
  char *buf = cee_load_whole_file_fp(fp, &buf_size);
  logconf_setup_sb(conf, id, buf, buf_size);
}
