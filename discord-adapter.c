...
static void setopt_cb(struct ua_conn *conn, void *p_token) {
...
static void _discord_context_cleanup(struct discord_context *cxt) {
...
static void _discord_adapter_set_errbuf(struct discord_adapter *adapter,
                                        struct sized_buffer *body) {
  size_t len;
...
#if defined(CURL_MINE)
static void _discord_context_to_mime(curl_mime *mime, void *p_cxt) {
...
static bool _discord_adapter_get_info(struct discord_adapter *adapter,
                                      struct ua_info *info,
                                      int64_t *wait_ms) {
...
static ORCAcode _discord_adapter_run_sync(struct discord_adapter *adapter,
  ...
  do {
    ...
    case ORCA_OK: {
      ...
      struct sized_buffer info_body;

      ua_info_extract(conn, &info);
      retry = _discord_adapter_get_info(adapter, &info, &wait_ms);

      info_body = ua_info_get_body(&info);
      if (info.code != ORCA_OK) {
        _discord_adapter_set_errbuf(adapter, &info_body);
      }
      else if (attr->ret) {
        /* initialize ret */
        ...
        /* populate ret */
        if (attr->from_json)
          attr->from_json(info_body.start, info_body.size, attr->ret);
      }
...
static void _discord_context_reset(struct discord_context *cxt) {
  cxt->bucket = NULL;
  cxt->done = NULL;
  cxt->fail = NULL;
  cxt->body.buf.size = 0;
...
static bool _discord_context_timeout(struct discord_adapter *adapter,
                                     struct discord_context *cxt) {
...
static ORCAcode _discord_adapter_send(struct discord_adapter *adapter,
                                      struct discord_context *cxt) {
...
static ORCAcode _discord_adapter_check_timeouts(struct discord_adapter *adapter) {
  struct discord_context *cxt;
...
static ORCAcode _discord_adapter_send_single(struct discord_adapter *adapter,
                                             struct discord_bucket *b) {
  struct discord_context *cxt;
  QUEUE *q;
...
static ORCAcode _discord_adapter_send_batch(struct discord_adapter *adapter,
                                            struct discord_bucket *b) {
  struct discord_context *cxt;
...
static ORCAcode _discord_adapter_check_pending(struct discord_adapter *adapter) {
  struct discord_bucket *b;
...
static ORCAcode _discord_adapter_check_action(struct discord_adapter *adapter,
                                              struct CURLMsg *msg) {
  struct discord *client = CLIENT(adapter, adapter);
  struct discord_context *cxt;
  int64_t wait_ms = 0LL;
...
static void _discord_init(struct discord *new_client) {
  orca_global_init();
...
extern void logconf_setup_sb(struct logconf *conf, const char *id, char *buf, size_t buf_size);

struct discord*discord_sb_init(char *buf, size_t buf_size) {
  struct discord *new_client;
  new_client = calloc(1, sizeof *new_client);
  logconf_setup_sb(&new_client->conf, "DISCORD", buf, buf_size);
...
struct discord* discord_config_init(const char config_file[]) {
  size_t buf_size = 0;
  char *buf = cee_load_whole_file(config_file, &buf_size);
  return discord_sb_init(buf, buf_size);
}